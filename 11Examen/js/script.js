...
const heroes = () => {
    ...
    //contenedor donde se va a pintar las imagenes de marvel
        heroImg = document.getElementById('content');

    const getHeroData = async(characterName) => {
    //Se concatena el ts(timestamp), la privateKey, publicKey y se forma el hash encriptados en MD5, y se agrega al URL para solicitar los personajes
        const ts = Date.now(),
            hash = MD5(ts + privateKey + publicKey),
            URL= `http://gateway.marvel.com/v1/public/characters?ts=${ts}&apikey=${publicKey}&hash=${hash}`;
    //Se piden los datos y se transforman a json
